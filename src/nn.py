...
class Parameter(Tensor):
    def __init__(self, data, *, dtype=None): super().__init__(data, dtype=dtype, requires_grad=True)
...
class Module:
    ...
    def _named(self, prefix=""):
        for name, value in self.__dict__.items():
            ...
            if isinstance(value, Parameter): yield key, value
            elif isinstance(value, Module): yield from value._named(key)
            elif isinstance(value, (list, tuple)):
                for i, item in enumerate(value):
                    if isinstance(item, Parameter): yield f"{key}.{i}", item
                    elif isinstance(item, Module): yield from item._named(f"{key}.{i}")
    ...
    def modules(self):
        ...
        for value in self.__dict__.values():
            if isinstance(value, Module): yield from value.modules()
            elif isinstance(value, (list, tuple)):
                for item in value:
                    if isinstance(item, Module): yield from item.modules()
    def train(self, mode=True):
        for module in self.modules(): module.training = bool(mode)
        return self
...
class Linear(Module):
    def __init__(self, in_features, out_features, bias=True, seed=None):
        super().__init__(); rng = np.random.default_rng(seed); bound = 1 / np.sqrt(in_features)
        self.weight = Parameter(rng.uniform(-bound, bound, (out_features, in_features)))
        self.bias = Parameter(np.zeros(out_features)) if bias else None
    def forward(self, x): return x @ self.weight.T + (self.bias if self.bias is not None else 0)
...
class Embedding(Module):
    def __init__(self, num_embeddings, embedding_dim, seed=None):
        super().__init__(); self.weight = Parameter(np.random.default_rng(seed).normal(0, 1 / np.sqrt(embedding_dim), (num_embeddings, embedding_dim)))
    def forward(self, indices): return self.weight[np.asarray(indices.data if isinstance(indices, Tensor) else indices, dtype=int)]


class Dropout(Module):
    def __init__(self, p=.5, seed=None):
        if not 0 <= p < 1: raise ValueError("p must be in [0, 1)")
        super().__init__(); self.p, self.rng = p, np.random.default_rng(seed)
    def forward(self, x): return x if not self.training or self.p == 0 else x * Tensor(self.rng.random(x.shape) >= self.p) / (1 - self.p)


class LayerNorm(Module):
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True):
        super().__init__(); shape = (normalized_shape,) if isinstance(normalized_shape, int) else tuple(normalized_shape); self.shape, self.eps = shape, eps
        self.weight = Parameter(np.ones(shape)) if elementwise_affine else None; self.bias = Parameter(np.zeros(shape)) if elementwise_affine else None
    def forward(self, x):
        axes = tuple(range(-len(self.shape), 0)); y = (x - x.mean(axes, True)) / (x.var(axes, True) + self.eps).sqrt()
